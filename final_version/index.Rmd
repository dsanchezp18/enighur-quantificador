...
date: '2026-06-01'
slug: 2026-06-01-enighur-ingresos-gastos-ecuador
authors:
...
tags:
  ...
  - gastos
  - hogares
description: 'Una lectura cuantitativa de la nueva ENIGHUR 2024-2025: cómo ganan, gastan y se diferencian los hogares ecuatorianos por nivel de ingreso, provincia y región.'
bibliography: references.bib
---

```{r, include=FALSE}
include_preferred_graphic <- function(paths) {
  candidate <- paths[file.exists(paths)][1]

  if (is.na(candidate)) {
    stop(
      "No graphic found. Checked:\n- ",
      paste(paths, collapse = "\n- "),
      call. = FALSE
    )
  }

  knitr::include_graphics(candidate)
}
```
...
La ENIGHUR es especialmente importante porque es la única encuesta con información detallada sobre el gasto de los hogares. Una de las estadísticas más importantes, la inflación, depende del porcentaje de gastos dedicados a cada uno de los componentes del índice de precios al consumidor. Sin esto, es imposible calcular una tasa de inflación representativa para el consumidor promedio. La última ENIGHUR se realizó hace más de una década, entre 2011 y 2012. Sin estos datos actualizados, la inflación en todos estos años, particularmente en los últimos, podría estar mal calculada.

En su extremo detalle, la ENIGHUR también nos permite entender tópicos especializados que han surgido en los últimos años. ¿Qué alimentos son los preferidos por el ecuatoriano? ¿Las familias de la Sierra consumen distinto que las de la Costa? En este artículo cubro los patrones más notables de la ENIGHUR y muestro varios detalles que ayudan a entender mejor a la sociedad ecuatoriana.

# El hogar ecuatoriano promedio gana $889 y gasta el 77% de ese ingreso

La encuesta nos informa que, durante 2025[^1], el promedio del ingreso monetario de los hogares fue de $889[^2]. Este promedio es un poco más optimista que el de la ENEMDU, dependiendo de cómo se lo calcule[^3]. El hogar ecuatoriano promedio tuvo 3.31 personas en 2025.

El gráfico 1 muestra cómo el ingreso de los hogares se distribuye en diferentes tipos de gasto para el hogar promedio. La gran mayoría de este ingreso se dedica a gasto corriente, con un promedio de $642 (72.2% del total). El gasto corriente es gasto de corto plazo[^4]. Un pequeño porcentaje se dedica a gasto de no consumo, es decir, impuestos, pensiones alimenticias, matriculaciones y transferencias a otros hogares o instituciones. Además, determino que:
...
- El ahorro o diferencia entre ingresos y gastos promedio es de $205, representando el 23.1% del ingreso[^5].

**Gráfico 1: Descomposición del ingreso monetario mensual del hogar promedio**

```{r, echo=FALSE, out.width="100%"}
include_preferred_graphic(c("images/grafico1.svg", "images/grafico1.png"))
```

*Fuente: ENIGHUR 2024-2025, INEC. Nota: El gráfico muestra cómo se reparte el ingreso monetario mensual del hogar promedio. El ahorro se calcula como la diferencia entre el ingreso monetario y los gastos del hogar.*

# La mayoría de hogares en Ecuador ganan menos de $1,000 mensuales

La mediana del ingreso según ENIGHUR fue de $620, lo que quiere decir que el 50% de los hogares gana hasta ese valor y el otro 50% gana más. Como el promedio nacional es mucho más alto, de $889, esto ya anticipa una distribución sesgada hacia arriba: hay relativamente pocos hogares con ingresos altos, pero empujan el promedio hacia arriba. El gráfico 2 muestra esa distribución solo para 2025.

**Gráfico 2: Distribución del ingreso monetario mensual de los hogares en 2025**

```{r, echo=FALSE, out.width="100%"}
include_preferred_graphic(c("images/grafico2.svg", "images/grafico2.png"))
```
...
- También he hablado en semanas pasadas de cuántos hogares ganan $1,500 o más. La ENIGHUR confirma, con claridad, lo que también sugiere la ENEMDU: solo 14.7% de los hogares llega a ese nivel.
...
```{r, echo=FALSE, out.width="100%"}
include_preferred_graphic(c("images/grafico3.svg", "images/grafico3.png"))
```

*Fuente: ENIGHUR 2024-2025, INEC. Nota: Q1 corresponde al quintil de menores ingresos y Q5 al de mayores ingresos. El gráfico resume el nivel típico de gasto monetario total mensual del hogar en cada grupo.*
...
```{r, echo=FALSE, out.width="100%"}
include_preferred_graphic(c("images/grafico4.svg", "images/grafico4.png"))
```

*Fuente: ENIGHUR 2024-2025, INEC. Nota: Gasolina incluye eco país, extra y súper. Las etiquetas muestran el gasto promedio mensual y su peso dentro del gasto monetario del hogar.*
...
```{r, echo=FALSE, out.width="100%"}
include_preferred_graphic(c("images/grafico5.svg", "images/grafico5.png"))
```
...
La distancia entre Galápagos y el resto sirve para recordar que el costo de vida no es homogéneo dentro del país. La ENIGHUR nos permite observar esta información por primera vez en muchos años, puesto que el índice de precios espacial de Galápagos no se ha realizado en una década. Si bien el gasto no es igual que el precio, porque gasto es precio por cantidad, es la mejor medida disponible para entender los costos de vida en el Ecuador por el momento. Es decir, la canasta es más cara.
...
```{r, echo=FALSE, out.width="100%"}
include_preferred_graphic(c("images/grafico6.svg", "images/grafico6.png"))
```
...
Los datos nos demuestran, o al menos verifican, que en nuestro país las familias consumen ciertas cosas de manera uniforme. Sin embargo, la composición regional varía: la Costa come cebolla, tomate y verde; la Sierra come pan; y la Amazonía se parece más a una mezcla de ambos patrones, con un peso especialmente alto del plátano verde.
...
Este artículo utiliza los resultados de la ENIGHUR 2024-2025 para describir patrones de ingreso y gasto de los hogares ecuatorianos. Los montos monetarios reportados corresponden a agregados mensuales por hogar, y las comparaciones por quintil, provincia y región se construyen a partir de las variables armonizadas en los scripts de análisis de este repositorio.

Cuando hablo de ahorro, no me refiero a una pregunta directa de la encuesta, sino a una variable derivada como diferencia entre ingreso monetario y gasto total del hogar. Además, para varios resultados privilegio medianas y percentiles por encima de promedios, porque la distribución del ingreso y del gasto está sesgada hacia arriba y los hogares de ingresos altos mueven marcadamente las medias nacionales.

[^1]: El periodo de recolección de datos de la encuesta fue de diciembre de 2024 a noviembre de 2025.
[^2]: INEC reporta un ingreso más alto con la misma encuesta, puesto que también considera ingresos no monetarios mediante estimaciones del valor de dichos ingresos en especie.
[^3]: Ya que la ENIGHUR es una encuesta optimizada tanto para medir el ingreso como el gasto, el valor de $889 es la mejor estimación disponible para este periodo.
[^4]: Formalmente, gasto en bienes y servicios dedicado a satisfacer necesidades y deseos cotidianos.
[^5]: La ENIGHUR no pregunta directamente sobre el ahorro; este valor se calcula como la diferencia entre ingreso monetario y gastos del hogar. Para entender mejor las tendencias de los valores monetarios, es preferible complementar el análisis con medianas y percentiles.
